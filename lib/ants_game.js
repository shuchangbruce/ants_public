...
class AntColony {
  ...
  deployAnt(place, ant){
    if(this._food >= ant.foodCost){
      ...
      if(ant.place === place) { //if successful 
...
var Ants = {
  ...
  Hungry : class extends Ant {
    ...
    act() {
      ...
      if(this._appetite == 0 && theBee) { //if the appetite is zero, eat the bee
        console.log(this.toString() + ' eats ' + theBee.name);
        this.place._removeInsect(theBee);
        this._appetite++;
      } else if(this._appetite > 0) { //if not zero, means there is some thing in it. digest it
        this._appetite++;
      }
      if(this._appetite == 4) {//when appetite reaches 4, means there has been 4 rounds. Rest appetite to zero
        this._appetite = 0;
      } 
    }
  },
